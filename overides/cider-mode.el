;;; overides/cider-mode.el -*- lexical-binding: t; -*-
...
(defvar font-lock-clj-core-function-name-face 'font-lock-clj-core-function-name-face
  "Face name to use for clojure core function names.")
(defvar font-lock-clj-core-macro-name-face 'font-lock-clj-core-macro-name-face
  "Face name to use for fclojure core variable names.")
(defvar font-lock-clj-core-var-name-face 'font-lock-clj-core-var-name-face
  "Face name to use for clojure core var names.")
...
(defun cider--compile-font-lock-keywords (symbols-plist core-plist)
  ...
  (let ((cider-font-lock-dynamically (if (eq cider-font-lock-dynamically t)
        ...
        macros functions vars instrumented traced core-functions core-macros core-vars)
    (cl-labels ((handle-plist
                     ...
                     (let ((sym (pop plist))
                       ...
                       (let ((is-macro (nrepl-dict-get meta "macro"))
                         ...
                         (cond ((and do-macro is-macro)
                                (if core?
                                    (push sym core-macros)
                                  (push sym macros)))
                               ((and do-function is-function)
                                (if core?
                                    (push sym core-functions)
                                  (push sym functions)))
      ...
      (when (memq 'core cider-font-lock-dynamically)
        (let ((cider-font-lock-dynamically '(function var macro core deprecated)))
          (handle-plist core-plist t)))
      (handle-plist symbols-plist nil))
    `(
      ,@(when macros
      ...
      ,@(when core-macros
          `((,(concat (rx (or "(" "#'")) ; Can't take the value of macros.
                      "\\(" (regexp-opt macros 'symbols) "\\)")
             1 (cider--unless-local-match font-lock-clj-core-macro-name-face))))
      ,@(when functions
          `((,(regexp-opt functions 'symbols) 0
             (cider--unless-local-match font-lock-function-name-face))))
      ,@(when core-functions
          `((,(regexp-opt functions 'symbols) 0
             (cider--unless-local-match font-lock-clj-core-function-name-face))))
      ,@(when vars
          `((,(regexp-opt vars 'symbols) 0
             (cider--unless-local-match font-lock-variable-name-face))))
      ,@(when core-vars
          `((,(regexp-opt vars 'symbols) 0
             (cider--unless-local-match font-lock-clj-core-variable-name-face))))
      ,@(when deprecated
